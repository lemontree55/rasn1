  ...
  describe NumericString do
    ...
    describe '#initialize' do
      it 'creates a NumericString with default values' do
        numeric = NumericString.new(:numeric)
    ...
    describe '#to_der' do
      ...
      it 'generates a DER string according to ASN.1 class' do
        numeric = NumericString.new(:numeric, class: :context)
        numeric.value = '8'
        expect(numeric.to_der).to eq(binary("\x92\x018"))
      end

      it 'generates a DER string according to default' do
        numeric = NumericString.new(:numeric, default: '123')
        numeric.value = '123'
      ...
      it 'generates a DER string according to optional' do
        numeric = NumericString.new(:numeric, optional: true)
        numeric.value = nil
        expect(numeric.to_der).to eq('')
        numeric.value = '123'
    ...
    describe '#parse!' do
      let(:numeric) { NumericString.new(:numeric) }
