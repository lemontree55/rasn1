  ...
  describe GeneralizedTime do
    ...
    describe '#to_der' do
      it 'generates a DER string' do
        ...
        gt.value = Time.new(2017, 12, 30, 16, 30, 0, 0)
    ...
    describe '#parse!' do
      ...
      it 'parses a DER GeneralizedTime (UTC, hours only)' do
        ...
        expect(gt.value).to eq(Time.utc(2017, 12, 30, 12, 0, 0))
        gt.parse!("\x18\x0e2017123012.25Z")
        expect(gt.value).to eq(Time.utc(2017, 12, 30, 12, 15, 0))
        gt.parse!("\x18\x102017123012.255Z")
        expect(gt.value).to eq(Time.utc(2017, 12, 30, 12, 15, 18))
      end

      it 'parses a DER GeneralizedTime (UTC, hours and minutes only)' do
        gt.parse!("\x18\x0d201712301233Z")
        expect(gt.value).to eq(Time.utc(2017, 12, 30, 12, 33, 0))
      ...
      it 'parses a DER GeneralizedTime (UTC, hours, minutes and seconds)' do
        ...
        gt.parse!("\x18\x1420171230123426.0002Z")
        expect(gt.value).to eq(Time.utc(2017, 12, 30, 12, 34, 26 + '0.0002'.to_r))
      end
      ...
      it 'parses a DER GeneralizedTime (local time, hours only)' do
        ...
        expect(gt.value).to eq(Time.local(2017, 12, 30, 12, 15, 1, 80_000))
      end
      ...
      it 'parses a DER GeneralizedTime (local time with difference with UTC, hours only)' do
        ...
        expect(gt.value).to eq(Time.new(2017, 12, 30, 12, 0, 0, '+02:00'))
        gt.parse!("\x18\x112017123012.5+0200")
        expect(gt.value).to eq(Time.new(2017, 12, 30, 12, 30, 0, '+02:00'))
        gt.parse!("\x18\x142017123012.255+0200")
        expect(gt.value).to eq(Time.new(2017, 12, 30, 12, 15, 18, '+02:00'))
      end
      ...
      it 'parses a DER GeneralizedTime (local time with difference with UTC, hours and minutes only)' do
        ...
        expect(gt.value).to eq(Time.utc(2017, 12, 30, 10, 56, 0))
      ...
      it 'parses a DER GeneralizedTime (local time with difference with UTC, hours, minutes and seconds)' do
        ...
        expect(gt.value).to eq(Time.new(2017, 12, 30, 12, 56, 45, '-10:00'))
        gt.parse!("\x18\x1720171230125645.123-0400")
        expect(gt.value).to eq(Time.new(2017, 12, 30, 12, 56, 45 + 123/1000r, '-04:00'))
      end
    end
  end
end
